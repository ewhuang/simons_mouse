...
def main():
    if len(sys.argv) != 2:
        print 'Usage:python %s sampled/full' % sys.argv[0]
        exit()
    size = sys.argv[1]
    assert size in ['sampled', 'full']

    if size == 'sampled':
        sampled_genes = file_operations.get_sampled_genes()
    matrix_gene_ids = parse_matrix_gene_ids()
    f = open(data_folder + 'Mouse.embedding', 'r')
    out = open('./data/embedding_%s_network.txt' % size, 'w')
    for i, line in enumerate(f):
        row_gene = matrix_gene_ids[i]
        if size == 'sampled' and row_gene not in sampled_genes:
            continue
        line = line.split()
        for j in range(i + 1, len(line)):
            column_gene = matrix_gene_ids[j]
            if size == 'sampled' and column_gene not in sampled_genes:
                continue
            weight = line[j]
